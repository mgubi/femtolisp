...
static value_t fl_time_now(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    (void)args;
    return mk_double(fl_ctx, clock_now());
}

static double todouble(fl_context_t *fl_ctx, value_t a, char *fname)
{
    if (isfixnum(a))
...
static value_t fl_##name(fl_context_t *fl_ctx, value_t *args, u_int32_t nargs)  \
{                                                                               \
    argcount(fl_ctx, #name, nargs, 1);                                          \
    if (iscprim(args[0])) {                                                     \
        cprim_t *cp = (cprim_t*)ptr(args[0]);                                   \
        numerictype_t nt = cp_numtype(cp);                                      \
        if (nt == T_FLOAT)                                                      \
            return mk_float(fl_ctx, name##f(*(float*)cp_data(cp)));             \
    }                                                                           \
    return mk_double(fl_ctx, name(todouble(fl_ctx, args[0], #name)));           \
}
